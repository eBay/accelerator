...
ZLIB_PREFIX="/io/zlib-ng-$AUDITWHEEL_PLAT"
...
for V in $(ls /opt/python/); do
	case "$V" in
		cp27-*|cp3[5-9]-*)
			...
			FIXED_NAME="/io/wheelhouse/$NAME-$V-$AUDITWHEEL_PLAT.whl"
			rm -f "$UNFIXED_NAME" "$FIXED_NAME"
			ACCELERATOR_BUILD_STATIC_ZLIB="$ZLIB_PREFIX/lib/libz.a" \
			CPPFLAGS="-I$ZLIB_PREFIX/include" \
			"/opt/python/$V/bin/pip" wheel /io/accelerator/dist/"$NAME".tar.gz --no-deps -w unfixed_wheels/
